...
public class TestTST {
	...
	public static void main(String[] args) {
		DataTST tree = new DataTST();
		int[] numbers = new int[]{10, 3, 1, 2, 15, 11, 20, 7, 9};
		for (int t = 0; t < 3; t++) {
			for (int i = 0; i < numbers.length; i++) {
				tree.add(new DataTST.Data(numbers[i], i + 100 * t));
			}
		}
		tree.printInorder();
		...
		for (DataTST.Data d : tree){
			System.out.print(d + " ");
		}
		System.out.println();

		Iterator iterator = tree.iterator();
		DataTST.Data d = (DataTST.Data) iterator.next();
		int oldNumber1 = d.number1;
		int oldNumber2 = d.number2;
		while (iterator.hasNext()) {
			DataTST.Data dd = (DataTST.Data) iterator.next();
...
// A TST class based on code from UW CSE 143, IntTree.  This will be modified
...
class DataTST implements Iterable<DataTST.Data> {
